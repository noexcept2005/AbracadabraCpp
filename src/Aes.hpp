#pragma once
/*
 * Copyright (C) 2025 SheepChef (a.k.a. Haruka Hokuto)
 *
 * 这是一个自由软件。
 * 在遵守AIPL-1.1许可证的前提下，
 * 你可以自由复制，修改，分发，使用它。
 *
 * 查阅 Academic Innovation Protection License(AIPL) 来了解更多 .
 * 本作品应随附一份完整的 AIPL-1.1 许可证全文。
 *
 */

#include <array>
#include <cstdint>
#include <vector>

namespace moyue::crypto {

namespace detail {
constexpr std::array<std::uint8_t, 256> kSbox = {
    };

constexpr std::array<std::uint8_t, 11> kRcon = {0x00, 0x01, 0x02, 0x04, 0x08,
                                                0x10, 0x20, 0x40, 0x80, 0x1B,
                                                0x36};

inline std::uint8_t Gmul(std::uint8_t a, std::uint8_t b) {
  std::uint8_t p = 0;
  for (int i = 0; i < 8; ++i) {
    if (b & 1) {
      p ^= a;
    }
    bool hi = (a & 0x80) != 0;
    a <<= 1;
    if (hi) {
      a ^= 0x1b;
    }
    b >>= 1;
  }
  return p;
}
}  // namespace detail

inline void Aes256KeyExpansion(const std::array<std::uint8_t, 32>& key,
                               std::array<std::uint8_t, 240>& roundKeys) {
  for (size_t i = 0; i < 32; ++i) {
    roundKeys[i] = key[i];
  }
  size_t bytesGenerated = 32;
  std::uint8_t rconIter = 1;
  std::array<std::uint8_t, 4> temp{};

  while (bytesGenerated < roundKeys.size()) {
    for (size_t i = 0; i < 4; ++i) {
      temp[i] = roundKeys[bytesGenerated - 4 + i];
    }
    if (bytesGenerated % 32 == 0) {
      std::uint8_t t = temp[0];
      temp[0] = temp[1];
      temp[1] = temp[2];
      temp[2] = temp[3];
      temp[3] = t;
      for (auto& val : temp) {
        val = detail::kSbox[val];
      }
      temp[0] ^= detail::kRcon[rconIter++];
    } else if (bytesGenerated % 32 == 16) {
      for (auto& val : temp) {
        val = detail::kSbox[val];
      }
    }

    for (size_t i = 0; i < 4; ++i) {
      roundKeys[bytesGenerated] = roundKeys[bytesGenerated - 32] ^ temp[i];
      ++bytesGenerated;
    }
  }
}

inline void AesEncryptBlock(const std::array<std::uint8_t, 16>& input,
                            std::array<std::uint8_t, 16>& output,
                            const std::array<std::uint8_t, 240>& roundKeys) {
  std::array<std::uint8_t, 16> state = input;

  auto addRoundKey = [&](size_t round) {
    size_t offset = round * 16;
    for (size_t i = 0; i < 16; ++i) {
      state[i] ^= roundKeys[offset + i];
    }
  };

  auto subBytes = [&]() {
    for (auto& val : state) {
      val = detail::kSbox[val];
    }
  };

  auto shiftRows = [&]() {
    std::array<std::uint8_t, 16> temp = state;
    state[1] = temp[5];
    state[5] = temp[9];
    state[9] = temp[13];
    state[13] = temp[1];

    state[2] = temp[10];
    state[6] = temp[14];
    state[10] = temp[2];
    state[14] = temp[6];

    state[3] = temp[15];
    state[7] = temp[3];
    state[11] = temp[7];
    state[15] = temp[11];
  };

  auto mixColumns = [&]() {
    for (size_t i = 0; i < 4; ++i) {
      size_t col = i * 4;
      std::uint8_t a0 = state[col];
      std::uint8_t a1 = state[col + 1];
      std::uint8_t a2 = state[col + 2];
      std::uint8_t a3 = state[col + 3];
      state[col] =
          detail::Gmul(a0, 2) ^ detail::Gmul(a1, 3) ^ a2 ^ a3;
      state[col + 1] =
          a0 ^ detail::Gmul(a1, 2) ^ detail::Gmul(a2, 3) ^ a3;
      state[col + 2] =
          a0 ^ a1 ^ detail::Gmul(a2, 2) ^ detail::Gmul(a3, 3);
      state[col + 3] =
          detail::Gmul(a0, 3) ^ a1 ^ a2 ^ detail::Gmul(a3, 2);
    }
  };

  addRoundKey(0);
  for (size_t round = 1; round < 14; ++round) {
    subBytes();
    shiftRows();
    mixColumns();
    addRoundKey(round);
  }
  subBytes();
  shiftRows();
  addRoundKey(14);

  output = state;
}

inline std::vector<std::uint8_t> Aes256CtrEncrypt(
    const std::vector<std::uint8_t>& data,
    const std::array<std::uint8_t, 32>& key,
    const std::array<std::uint8_t, 16>& iv) {
  std::array<std::uint8_t, 240> roundKeys{};
  Aes256KeyExpansion(key, roundKeys);

  std::array<std::uint8_t, 16> counter = iv;
  std::vector<std::uint8_t> output(data.size());
  size_t offset = 0;

  auto incrementCounter = [&]() {
    for (int i = 15; i >= 0; --i) {
      if (++counter[static_cast<size_t>(i)] != 0) {
        break;
      }
    }
  };

  while (offset < data.size()) {
    std::array<std::uint8_t, 16> stream{};
    AesEncryptBlock(counter, stream, roundKeys);
    size_t blockLen = std::min<size_t>(16, data.size() - offset);
    for (size_t i = 0; i < blockLen; ++i) {
      output[offset + i] =
          static_cast<std::uint8_t>(data[offset + i] ^ stream[i]);
    }
    offset += blockLen;
    incrementCounter();
  }
  return output;
}

}  // namespace moyue::crypto
